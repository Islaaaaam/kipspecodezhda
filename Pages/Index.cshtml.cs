...
namespace SleekClothing.Pages
{
    public class IndexModel : PageModel
    {
        ...
        public IActionResult OnPostAddToCart(int productId)
        {
            ...
            {
                TempData["error"] = $"{product.Name} не в наличии.";
                return Redirect("/");
            }
            ...
            TempData["success"] = $"{product.Name} успешно добавлен(ы) в корзину!";
            return Redirect("/");
        }
        ...
        public IActionResult OnPostAddToWishlist(int productId)
        {
            try
            {
                ...
                TempData["success"] = $"{product.Name} добавлен(ы) в ваш вишлист.";
                return Redirect("/");
            }
            catch
            {
                TempData["error"] = $"Авторизуйтесь, чтобы добавлять товары в корзину.";
                return Redirect("/");
            }
        }
    }
